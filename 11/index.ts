...
if (filestat.isDirectory()) {
  readdirSync(inputPath).forEach(async (file) => {
    ...
    const compileWritePath = jackReadPath.replace(".jack", ".vm");
    const tokens = await tokenize(jackReadPath);
    const vmCode = compile(parse(tokens));
    const writer = createWriteStream(compileWritePath);
    writer.write(vmCode);
  });
  ...
  (async () => {
    ...
    const compileWritePath = inputPath.replace(".jack", ".vm");
    const tokens = await tokenize(inputPath);
    const vmCode = compile(parse(tokens));
    const writer = createWriteStream(compileWritePath);
    writer.write(vmCode);
  })();
